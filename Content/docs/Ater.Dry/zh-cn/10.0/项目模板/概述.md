...
Perigon.template 是基于`ASP.NET Core`和`EntityFramework Core`技术栈的项目模板，提供了开箱即用的项目结构和基础功能，帮助开发者快速搭建一个现代化的Web应用程序。
...
- Ater.AspNetCore：Web开发相关公共部分，包含基础模型，常用扩展方法和工具帮助类等。
- Ater.AspNetCore.Toolkit：在Web开发中常用的三方类库集成，如发邮件，图形验证码，Excel导出等。
- Ater.AspNetCore.SourceGeneration：源代码生成器和代码分析器相关功能。

由于框架层与项目和业务无关，它们可以被打包成类库，为此我们提供了打包后的Nuget包，包括：

- Ater.AspNetCore
- Ater.AspNetCore.Toolkit
- Ater.AspNetCore.SourceGeneration
...
模板中关于`Aspire`的内容，一个是`AppHost`项目，一个是`Definition`目录下的`ServiceDefaults`项目。
