...
通过`studio`中的模块管理界面，可以非常方便地创建和删除模块，工具会自动跨程序集处理模块相关的依赖关系。
...
要进行打包，需要编辑`ModuleExtensions.cs`文件，该文件在创建模块时自动生成，如：
...
使用`perigon module pack <模块名称> <服务名称>`命令进行打包，必须在项目根目录下执行，打包后将在根目录下生成`package_modules`目录。需要打包前端模块时，使用 `--front-path` 指定该模块目录；详细规则见[命令行文档](../代码生成/命令行.md)。
