...
当目标类型为 `csharp` 时，OpenAPI 中的 `multipart/form-data` 文件字段会生成文件上传方法。生成的方法使用 `Stream` 接收文件，并额外接收文件名；OpenAPI schema 中的字段名会作为 multipart 字段名传入。其他表单字段和查询参数仍按 OpenAPI 定义生成。
