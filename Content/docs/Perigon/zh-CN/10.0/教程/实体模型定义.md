...
- 所有实体类默认继承自`EntityBase`，该类定义了`Id`、`CreatedTime`、`UpdatedTime`,`IsDeleted`常用属性。
...
- `DbContext`中的`DbSet`属性使用标准的`{ get; set; }`定义；除非有明确需求，不要额外使用`ToTable`等配置。
...
- `TenantId`用于业务实体的租户归属，`EntityBase`默认实现`ITenantEntityBase`。`Tenant`是全局租户目录根实体，会在`ContextBase`中忽略继承的`TenantId`。
...
由于系统统一使用租户感知模型，所以由`TenantIndexConvention`在模型最终确定阶段自动处理`TenantId`索引。你不需要为每个实体手动添加`TenantId`到唯一约束中（手动添加也不会重复），系统会自动将它放在索引首列；对于唯一约束，还会自动添加过滤条件以排除软删除数据。该规则不依赖`IsMultiTenant`配置。


> [!TIP]
> 如需修改默认实现，请调整`EntityFramework/AppDbContext/TenantIndexConvention`，并同步验证生成的迁移内容。
...
可以使用以下命令输出供 LLM 参考的实体模型生成规则：

```pwsh
perigon generate entity
```

该命令只输出规则，不会直接创建实体文件。实体应放在 `src/Definition/Entity` 项目中；指定模块时，应在实体项目下创建模块目录，目录名为模块名称加 `Mod`（例如 `UserMod`）。如果模块尚不存在，应先创建模块，再创建实体模型。
