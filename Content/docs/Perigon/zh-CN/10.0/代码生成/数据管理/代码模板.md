...
> [!IMPORTANT]
> **10.2.0 变更：** Studio 已移除代码模板管理页面。以下内容完整保留，用于说明 10.2.0 之前版本的功能与操作，不适用于当前版本。旧 `templates/*.razor` 文件不会被升级命令主动删除；新的项目专用生成规则应维护在项目 skills 中。
...
> [!TIP]
> 编写模板不是件容易的事情，你需要了解你可以使用哪些变量和数据结构，可借助[MCP工具来生成](../MCP%20Server/内置工具使用示例.md#生成Razor模板)，并在此基础上进行修改.
