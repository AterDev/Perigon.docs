...
我们使用`Mapster`来进行实体类与DTO之间的转换。DTO的内容是根据实体类生成的，这是前置的条件。
...
> [!TIP]
> 由于使用`EntityFrameworkCore.Design`来获取实体信息，那些不被认为是数据库映射的属性，在生成添加/修改DTO时，不会包含这些属性。
>
