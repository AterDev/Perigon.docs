...
- `src/AppHost/appsettings*.json`：由 AppHost 读取，用于选择数据库、缓存和多租户模式，并在 Aspire 启动时统一注入服务。
...
| `Components:IsMultiTenant` | `Components__IsMultiTenant` | 是否启用多租户 |
...
| `IsMultiTenant` | 布尔值 | 启用租户解析、中间件和租户数据隔离。 |
...
- `Components__IsMultiTenant`：决定迁移差异处理是否生成租户相关索引。
