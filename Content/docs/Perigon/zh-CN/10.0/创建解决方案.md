...
- 修改 `Definition/EntityFramework` 中的 `ContextBase` 或 `AppDbFactory`，以支持您需要的数据库类型。
...
- `Perigon.ResourceMod`：按环境、分类、资源定义与角色权限管理通用资源。

各模块的能力边界与安装说明见[官方模块](./项目模板/模块示例.md)。

如果当前网络环境无法访问官方模块清单，创建流程仍可继续，只是这一轮无法直接选择官方模块。

> [!TIP]
> 官方模块本质上仍然是模块包。即使创建时没有选择，也可以在解决方案创建完成后，再使用 `perigon module install Perigon.SystemMod <服务名称>` 这样的命令补充安装。
