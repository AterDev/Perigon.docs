...
Runtime components integrated through Aspire. The template references `Perigon.AspNetCore.Toolkit` to provide helper capabilities such as email, object storage, and Excel. Whether a capability is enabled depends on configuration and application code:
