...
如果需要系统管理、内容管理等现成能力，可以在创建时直接勾选官方模块。工具会在生成解决方案后自动安装所选模块。
...
最新模板不再默认包含 `SystemMod` 等示例模块。如果在创建解决方案时没有选择官方模块，而后续又需要这些能力，可以再通过 `perigon module install Perigon.SystemMod <服务名称>` 进行安装。模块说明见[官方模块](./项目模板/模块示例.md)。

关于目录的详细说明请参考[目录结构](./项目模板/目录结构.md)。

> [!NOTE]
> 官方模块安装后，如需移除，建议通过 Studio 删除，它会同时处理模块相关引用与依赖。
...
- AdminService：提供后台管理需要的接口服务。如果你安装了 `Perigon.SystemMod`，首次运行时会初始化管理员账号和密码。
