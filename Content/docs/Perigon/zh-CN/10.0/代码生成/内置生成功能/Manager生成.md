...
Manager是业务逻辑层的主要实现部分，内置工具可以在此生成常见的 CRUD 代码。本文将说明Manager生成的细节。

**支持方式：`Studio`和`MCP`**

> [!WARNING]
> 内置 Manager 实体代码生成仅支持 Standard 类型项目。AOT 项目（解决方案根目录 `.config/perigon.config.toml` 中配置 `isAOT = true`）不支持此功能。
