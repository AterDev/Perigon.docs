...
你可以定义自己的`DbContext`，并继承`ContextBase`。数据库上下文集中在`Definition/EntityFrameworkCore/AppDbContext`目录下。
...
模板默认会使用`TenantDbFactory`来创建数据库上下文实例，以支持多租户场景，它会通过`ITenantContext`来获取 当前租户的信息，然后创建对应的数据库上下文。
