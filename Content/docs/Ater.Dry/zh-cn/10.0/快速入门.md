...
> [!TIP]
> 可以直接安装`VisualStudio 2026`IDE，选择`Asp.Net Core`工作负载，它已经集成了`.NET 10 SDK`和`.NET Aspire`组件。
...
> [!NOTE]
> 请使用`powershell 7`及以上版本，它是跨平台的，在Windows上你需要开启执行策略，运行`Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`命令来设置。

### 启动AppHost项目

接下来，让我们运行`AppHost`项目。你可以使用VS或命令`dotnt run`来运行该项目。但在此之前，你需要先确保`Docker`或`Podman`已经运行，以便Aspire能够拉取相应的镜像。
