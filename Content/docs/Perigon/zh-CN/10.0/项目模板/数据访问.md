...
你可以定义自己的`DbContext`，并继承`ContextBase`。数据库上下文集中在`Definition/EntityFramework/AppDbContext`目录下。

为了兼容多租户场景，默认将会使用`TenantDbFactory`来创建数据库上下文实例，如果你的应用不需要多租户支持，可以直接注入`DefaultDbContext`。
...
模板默认会使用`TenantDbFactory`来创建数据库上下文实例，以支持多租户场景。当前租户标识来自`IUserContext.TenantId`，Manager 基类会将该值传给`TenantDbFactory`，再由工厂选择默认连接字符串或租户独立连接字符串。
...
`UniversalDbFactory`默认会根据你的数据库上下文名称获取对应的连接字符串，并创建对应的`DbContext`实例，你可查看`UniversalDbFactory`中的代码逻辑，如：
