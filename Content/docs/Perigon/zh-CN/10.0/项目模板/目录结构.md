...
下面的目录说明以 `ApiStandard` 为主。`MiniApi` 不包含 `AdminService`、`MigrationService` 和默认 `Modules`，业务代码默认位于 `src/Services/ApiService` 的 `Endpoints/Managers/Models/Services` 目录。
...
- MigrationService：`ApiStandard`使用`EF Core`提供数据库迁移服务；`MiniApi`不包含该服务。
