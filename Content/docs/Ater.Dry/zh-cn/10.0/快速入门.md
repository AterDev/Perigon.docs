...
打开解决方案，在src目录下，可以看到相关项目。

找到`AppHost`项目，后续将主要使用它来运行项目。

模板默认包含了`SystemMod`模块，用来提供基础的用户/角色/权限管理功能，也是很好的示例代码。

关于目录的详细说明请参考[目录结构](./项目模板/目录结构.md)。

> [!NOTE]
> 如果不需要默认模块，可在熟悉整体结构后，删除相关的内容，通常包含**实体定义、模块、控制器等部分**.

## 运行项目

### 创建数据库迁移

首次运行项目，或对EF Core实体进行了更改，我们需要先创建数据库迁移。
...
如果`AdminService`是`Running`状态，那么表示正常运行，我们先看一下`swagger`是否正常，直接在浏览器输入`http://localhost:7001/swagger/v1/swagger.json`，查看是否能正常访问。
