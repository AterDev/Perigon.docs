...
当目标类型为 `csharp` 时，OpenAPI 中的 `multipart/form-data` 会生成 multipart 上传方法。单个文件字段使用 `MultipartFile`（封装 `Stream`、文件名和可选 MIME 类型），多个文件字段使用 `IEnumerable<MultipartFile>`；普通字符串、数字、布尔值和数组字段也会作为表单字段提交。Angular 和 Axios 客户端分别使用 `File`/`File[]`，并按 schema 字段名写入 `FormData`。
