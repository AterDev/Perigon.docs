...
- 无密码登录：通过发送验证码或设备确认等方式进行身份验证，无需密码。
...
当你的配置中包含这些身份验证方式并具有有效的配置时，模板会自动为你添加相应的服务和配置。

你可以在`WebExtensions.cs`中查看`AddJwtAuthentication`和`AddThirdAuthentication`方法的具体实现。