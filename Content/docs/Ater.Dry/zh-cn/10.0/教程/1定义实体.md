...
所有实体都建议继承`EntityBase`抽象类，它定义了一些基础属性，具体如下：
...
你可以根据自己的偏好，修改或删除某些属性。

需要注意的是，如果你修改了默认的Id或时间属性名称，需要同步`ContextBase`中的引用，DbContext会根据这些字段名实现自动赋值和软删除功能。

> [!TIP]
> 建议使用`Guid`作为主键类型，代码生成将默认使用`Guid`类型。
