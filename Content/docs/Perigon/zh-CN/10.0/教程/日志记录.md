...
ASP.NET Core 提供了内置的日志记录功能，支持多种日志提供程序。`Aspire`在此基础上，将日志信息通过`OpenTelemetry`标准导出到日志收集器。你可以通过`OTEL_EXPORTER_OTLP_ENDPOINT`环境变量来设置日志收集器的地址。
...
> [!TIP]
> 以上示例可在`SystemMod`中找到相关实现，你可参考来实现自己的日志记录服务。
