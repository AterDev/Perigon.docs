...
- `src/AppHost/appsettings*.json`：由 AppHost 读取，用于选择数据库、缓存和租户配置，在 Aspire 启动时统一注入服务。
...
| `Components:IsMultiTenant` | `Components__IsMultiTenant` | 传递租户配置；`ApiStandard`始终使用Tenant目录、TenantId字段、全局过滤器和保存校验。 |
...
| `IsMultiTenant` | 布尔值 | 租户配置。`ApiStandard`始终使用租户感知的数据模型；单租户也会初始化默认Tenant并为业务实体绑定TenantId。是否使用租户独立数据库连接由Tenant记录和`AppDbFactory`决定。 |
