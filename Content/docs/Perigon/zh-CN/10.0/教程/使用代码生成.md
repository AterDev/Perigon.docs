...
当选择 `Csharp` 客户端时，OpenAPI 的 `multipart/form-data` 会生成 multipart 上传方法。单个文件字段使用 `MultipartFile`，多个文件字段使用 `IEnumerable<MultipartFile>`；普通字段也会写入表单。Angular 和 Axios 客户端使用 `File`/`File[]` 构造 `FormData`。
