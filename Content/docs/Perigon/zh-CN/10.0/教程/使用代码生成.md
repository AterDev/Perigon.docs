...
> [!WARNING]
> 内置的 DTO、Manager 和 Controller 实体代码生成仅支持 Standard 类型项目。工具会读取解决方案根目录的 `.config/perigon.config.toml`；当其中配置 `isAOT = true` 时，将提示不支持 AOT 代码生成。

选择某个解决方案，进入工作台，我们将会看到实体列表，在操作列中有三个生成操作按钮，它们分别是:

- 生成DTO：通过实体解析生成对应的DTO模型定义
- 生成Manager：生成包含基础 CRUD 操作的Manager类，由于依赖DTO，会自动生成成DTO。
- 生成Controller：生成包含基础 CRUD 操作的Controller类，由于依赖Manager，会自动生成成Manager和DTO。
...
- `base.service.ts`：Angular/Axios 客户端默认保留已有文件中的自定义内容；命令行可以使用 `-c/--cover-base-service` 覆盖，Studio 默认保留且界面不提供覆盖开关。

当选择 `Csharp` 客户端时，OpenAPI 的 `multipart/form-data` 会生成 multipart 上传方法。单个文件字段使用 `MultipartFile`，多个文件字段使用 `IEnumerable<MultipartFile>`；普通字段也会写入表单。Angular 和 Axios 客户端使用 `File`/`File[]` 构造 `FormData`。C# 客户端会保留 `HttpClient.BaseAddress` 的多级路径。
